        ...
        <div className="container" style={{ paddingTop: 'var(--spacing-lg)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 'var(--spacing-xl)' }}>
                ...
                <div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: listing.images.length > 1 ? '1fr 1fr' : '1fr', gap: '10px', marginBottom: 'var(--spacing-md)' }}>
                        <div style={{ borderRadius: 'var(--radius-lg)', overflow: 'hidden', height: listing.images.length > 1 ? '400px' : '500px', gridColumn: listing.images.length > 1 ? 'span 2' : 'span 1' }}>
                            <img src={listing.images[0]} alt={listing.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        </div>
                        {listing.images.length > 1 && (
                            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(listing.images.length - 1, 4)}, 1fr)`, gap: '10px', gridColumn: 'span 2' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-md)', marginBottom: 'var(--spacing-lg)' }}>
                        <div style={{ width: '56px', height: '56px', borderRadius: '50%', overflow: 'hidden', backgroundColor: '#f3f4f6', border: '2px solid #e5e7eb' }}>
                            {listing.hostAvatar ? <img src={listing.hostAvatar} alt={listing.hostName} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <User size={30} style={{ margin: '13px' }} />}
                        </div>
                        <div>
                            <p style={{ fontWeight: 600, fontSize: '1.1rem' }}>
                                {t('details.hosted_by', { name: listing.businessName || listing.hostName })}
                            </p>
                            <Link to={`/business/${listing.hostId}`} style={{ fontSize: '0.9rem', color: 'var(--color-primary)', textDecoration: 'none', fontWeight: 600 }}>
                                View Business Page →
                            </Link>
                        </div>
                    </div>
                    ...
                    {listing.videos && listing.videos.length > 0 && (
                        <div style={{ marginBottom: 'var(--spacing-xxl)' }}>
                            <h3 style={{ marginBottom: 'var(--spacing-md)', fontSize: '1.5rem' }}>Videos</h3>
                            <div style={{ display: 'grid', gridTemplateColumns: listing.videos.length > 1 ? '1fr 1fr' : '1fr', gap: 'var(--spacing-md)' }}>
                                {listing.videos.map((vid, idx) => (
                                    <div key={idx} style={{ borderRadius: 'var(--radius-md)', overflow: 'hidden', backgroundColor: '#000' }}>
                                        <video src={vid} controls style={{ width: '100%', height: listing.videos.length === 1 ? '400px' : '250px', objectFit: 'contain' }}></video>
                                    </div>
                                ))}
                            </div>
                        </div>
